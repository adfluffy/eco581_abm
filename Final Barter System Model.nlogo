...
globals [
  ...
  happiness_rate_log
  happiness_per_trade
  happiness_per_trade_log
  max_happiness
  max_happiness_log
  min_happiness
  min_happiness_log
  delta_turn_happiness
  delta_turn_happiness_log
  delta_agg_happiness
  delta_agg_happiness_log
  trade_rate
  trade_rate_log
  gini
  gini_log
...
to setup ;; sets the initial conditions for the ABM
  ...
  set delta_turn_happiness_log []
  set delta_agg_happiness_log []
  set trade_log []
  set trade_rate_log []
  set happiness_rate_log []
  set happiness_per_trade_log []
  set gini_log []
  set max_happiness_log []
  set min_happiness_log []
...
to market
  ...
  set delta_turn_happiness agg_happiness - prev_agg_happiness
  set delta_turn_happiness_log fput delta_turn_happiness delta_turn_happiness_log

  set delta_agg_happiness max(agg_happiness_log) - min(agg_happiness_log)
  set delta_agg_happiness_log fput delta_agg_happiness delta_agg_happiness_log

  set happiness_rate delta_agg_happiness / ticks
  set happiness_rate_log fput happiness_rate happiness_rate_log

  set trade_rate trades_executed / ticks
  set trade_rate_log fput trade_rate trade_rate_log

  set gini gini-calculate
  set gini_log fput gini gini_log

  set max_happiness max [happiness] of turtles
  set min_happiness min [happiness] of turtles
  set max_happiness_log fput max_happiness max_happiness_log
  set min_happiness_log fput min_happiness min_happiness_log

  set max_happiness_rate max [turt_happiness_rate] of turtles
  set min_happiness_rate min [turt_happiness_rate] of turtles
...
to set-happiness
  ...
  set happiness_log fput happiness happiness_log

  if ticks > 0 [
    set turt_happiness_rate (max happiness_log - min happiness_log) / ticks
  ]
...
to offer-histogram
  ...
  if goods != 0 [
    ...
    set-plot-y-range 0 (round ((count turtles / temp) * 1.5) + 1)
...
to do-plots
  ...
  set-current-plot "Average Utility"
  plot avg_happiness
  set-plot-x-range 0 ticks
  set-plot-y-range (precision (0.9 * min avg_happiness_log) 2) max (list (precision (1.1 * max avg_happiness_log) 2) 0.1)

  set-current-plot "Max Utility"
  plot max_happiness
  set-plot-x-range 0 ticks
  set-plot-y-range (precision (0.9 * min max_happiness_log) 2) max (list (precision (1.1 * max max_happiness_log) 2) 0.1)

  set-current-plot "Min Utility"
  plot min_happiness
  set-plot-x-range 0 ticks
  set-plot-y-range (precision (0.9 * min min_happiness_log) 2) max(list (precision (1.1 * max min_happiness_log) 2) 0.1)

  set-current-plot "Gini Coefficient"
  plot gini
  set-plot-x-range 0 ticks
  set-plot-y-range (precision (0.9 * min gini_log) 2) max (list (precision (1.1 * max gini_log) 2) 0.1)

  set-current-plot "Trade Rate"
  plot trade_rate
  set-plot-x-range 0 ticks
  set-plot-y-range (precision (0.9 * min trade_rate_log) 2) max (list (precision (1.1 * max trade_rate_log) 2) 0.1)

  set-current-plot "Rate of Utility Change"
  plot happiness_rate
  set-plot-x-range 0 ticks
  set-plot-y-range min (list (precision (0.9 * min happiness_rate_log) 2) 0) max (list (precision (1.1 * max happiness_rate_log) 2) 0.1)
...
to-report gini-calculate
  ...
  set utility sort-by [ [a b] -> a < b ] utility
  set population_size length utility

  foreach utility [ x ->
    set i i + 1
    set gini_coe gini_coe + (i * (x - avg_happiness))

  ]

  set gini_coe gini_coe * ( 2 / (population_size * population_size * avg_happiness))
...
@#$#@#$#@
<experiments>
  <experiment name="Barter" repetitions="3" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>market</go>
    <metric>max_happiness</metric>
    <metric>min_happiness</metric>
    <metric>gini</metric>
    <metric>trade_rate</metric>
    <metric>avg_happiness</metric>
    <metric>delta_agg_happiness</metric>
    <enumeratedValueSet variable="System_Type">
      <value value="&quot;Barter&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="Max_Iterations" first="100" step="50" last="1000"/>
    <steppedValueSet variable="Initial_Agents" first="10" step="10" last="1500"/>
  </experiment>
  <experiment name="Monetary" repetitions="3" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>market</go>
    <metric>max_happiness</metric>
    <metric>min_happiness</metric>
