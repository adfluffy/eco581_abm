...
to trade-match ;; goods trade mechanics - checks for offer/ask match and adjusts each turtles' goods_set if a match is found
...
to turtle-maker [amount] ;; generates turtle and initial turtle variables
  ...
  create-turtles amount [ ;; calls the create-turtles function
    ...
    foreach goods_set [
    x ->
      set happiness (item 1 x * item 2 x) + happiness
    ]
